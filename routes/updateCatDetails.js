...
router.put(
  ...
  async (req, res) => {
    ...
    db.query("SELECT * FROM Cats WHERE catID = ?", [catID], (err, result) => {
      ...
      let sql =
        "UPDATE Cats SET name = ?, gender = ?, age = ?, color = ?, breed = ?, description = ?, catImgPath = ?, updateDateTime = ?, cwEmail = ? WHERE catID = ?";
      ...
      db.query(
        ...
        (err, result) => {
          if (err) {
            ...
            return res
              .status(200)
              .json({ message: "Details of the cat updated successfully!" });
          }
        }
      );
    });
