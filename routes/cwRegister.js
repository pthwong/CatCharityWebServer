...
router.post('/', async (req, res) => {
    ...
    try {
      ...
      db.query(sqlCheckCode, [hashedSignupCode], async (err, result) => {
        ...
        const sqlRegister = "INSERT INTO CharityWorker (cwName, cwEmail, cwPassword, signUpCode) VALUES (?, ?, ?, ?)";
      
        db.query(sqlRegister, [cwName, cwEmail, hashedCwPassword, hashedSignupCode], (err, result) => {
          if (err) {
            // Handle error
            res.status(500).json({ error: err.message });
          } else {
